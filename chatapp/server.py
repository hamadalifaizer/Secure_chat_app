...
async def broadcast(tag='message',text='', sender=SERVER_NAME):
    if tag == 'message':
        try :
            ...
            print(f"Broadcast Success Message sent to Client ")
            for target in MAILING_ADDRESS.keys():
                async with websockets.connect(f"ws://{MAILING_ADDRESS[target]}") as websocket:
        ...
        try:
            for target in MAILING_ADDRESS.keys():
                async with websockets.connect(f"ws://{MAILING_ADDRESS[target]}") as websocket:
                    ...
                    print("TAG :: ATTENDACE")
                    # if websocket.recv():
                        # await server_handler(websocket.recv())
...
def message_handle(message):
    ...
    if tag == "presence":
        print("Presence unpacking")
        predata = data['presence']
        for entry in predata:
            nickname = entry['nickname']
            publickey = entry['publickey']
            if nickname in MAILING_ADDRESS.keys():
                mailing_pubkey[nickname] = publickey
                print(f"Hello {nickname}")        

    elif tag == 'attendance':
            onlineclient = share_contact()
            text = {'tag': 'presence', 'presence': onlineclient}
        ...
        print(f"Handling {tag} from {sender} to {receiver}\n filename is {filename} ")
        text = {'from': sender, 'to': receiver, 'filename': filename, 'info': text}
        return text
        ...
        text = data['info'] 
        print(f"Handling {tag} from {sender} to {receiver}")
        text = {'from': sender, 'to': receiver, 'info': text}
        return text
            ...
            try:
                ...
                if tag == "presence":
                    ...
                    if sender:
                        ...
                        # Initialize sender in adress_book
                        adress_book[sender] = {}
                    ...
                    async with websockets.connect(f"ws://{websocket.remote_address[0]}:5555") as target_socket:
                        onlineclient = share_contact() 
        ...
        for target in MAILING_ADDRESS.keys():
            ...
            while attempt < 3 and not connected and server_check[target] == 'uncheck':
                try:
                    ...
                    attempt += 1
                    print(f"Attempt to connect to {target} : Attempt {attempt} ")
                    await asyncio.sleep(5)
            if not connected:
                print(f"Failed to connect to {target} after 3 attempts")
...
async def main():
    try:
        ...
        print("Broadcasted Alive to Servers")
